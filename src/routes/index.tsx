...
function CTAButton({
  ...
  const sizes =
    size === "lg"
      ? "px-7 py-4 text-sm md:text-base gap-3"
      : "px-5 py-3 text-xs gap-2";
  const iconWrapperSize = size === "lg" ? "w-6 h-6" : "w-5 h-5";
  ...
  return (
    <a
      ...
      className={`group relative inline-flex items-center justify-center rounded-2xl bg-gradient-to-b from-[#FF5C35] to-[#E54D2A] text-center font-bold uppercase tracking-wider text-white transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${sizes} ${className}`}
      style={{
        boxShadow: "0 0 40px -10px rgba(255, 92, 53, 0.4)",
        borderTop: "1px solid rgba(255, 255, 255, 0.2)",
      }}
    >
      {/* Glossy overlay */}
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-white/10 via-transparent to-transparent opacity-50" />

      {/* Text content */}
      <span className="relative z-10">{children}</span>

      {/* Icon wrapper */}
      <div className="relative z-10 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-sm transition-colors group-hover:bg-white/20">
        <ArrowRight className={`${iconSize} shrink-0 transition-transform duration-300 group-hover:translate-x-0.5`} />
      </div>

      {/* Bottom reflection */}
      <div className="absolute -bottom-1 left-1/2 w-4/5 -translate-x-1/2 h-px bg-gradient-to-r from-transparent via-[#C7FF3D]/30 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

      {/* Hover glow */}
      <div
        className="pointer-events-none absolute -inset-1 rounded-[18px] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        style={{ boxShadow: "0 0 50px -5px rgba(255, 92, 53, 0.6)" }}
      />
    </a>
  );
}
